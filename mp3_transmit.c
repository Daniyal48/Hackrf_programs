...
void cleanup(const char* wav_file) {
    hackrf_close(device);
    hackrf_exit();

    // Remove the generated WAV file if provided
    if (wav_file != NULL) {
        if (remove(wav_file) == 0) {
            printf("Removed %s\n", wav_file);
        } else {
...
void signal_handler(int signal) {
    if (signal == SIGINT) {
        printf("\nCtrl+C signal received. Stopping transmission...\n");
        stop_signal = 1;
		cleanup(NULL);
    }
}
...
int convert_mp3_to_wav(const char* mp3_file, const char* wav_file) {
    int ret = 0;
    ...
    if (mpg123_open(mh, mp3_file) != MPG123_OK) {
        fprintf(stderr, "Cannot open input MP3 file: %s\n", mp3_file);
        mpg123_delete(mh);
        mpg123_exit();
        return 1;
    }
    ...
    long rate;
    int channels;
    mpg123_getformat(mh, &rate, &channels, NULL);
    ...
    printf("Conversion successful. %s converted to %s\n", mp3_file, wav_file);
    return 0;
}
int main(int argc, char *argv[]) {
    ...
    if (argc == 2) {
        ...
        // Check the file extension to determine if it's an MP3 or WAV file
        char* file_extension = strrchr(argv[1], '.');
        if (file_extension != NULL && strcmp(file_extension, ".mp3") == 0) {
            // Convert the MP3 file to WAV before transmitting
            char wav_file[500];
            strcpy(wav_file, argv[1]);
            wav_file[strlen(argv[1]) - 3] = '\0';
            strcat(wav_file, "wav");
            if (convert_mp3_to_wav(argv[1], wav_file) == 0) {
				printf("%S",wav_file);
                init_hackrf();
                tx_hackrf(wav_file);
            } else {
				printf("%s",wav_file);
                printf("MP3 to WAV conversion failed. Exiting.\n");
                return 1;
            }
        } else if (file_extension != NULL && strcmp(file_extension, ".wav") == 0) {
            // Transmit the WAV file directly
            init_hackrf();
            tx_hackrf(argv[1]);
        } else {
            printf("Unsupported file format. Only MP3 and WAV files are supported.\n");
            printf("Usage: %s <MP3 or WAV File Abs Path>\n", argv[0]);
            return 1;
        }
    } else {
        printf("Usage: %s <MP3 or WAV File Abs Path>\n", argv[0]);
